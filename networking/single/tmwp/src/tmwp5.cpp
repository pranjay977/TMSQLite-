//lc112
//MAIN CONTAINT OF THIS CODE ( at line 422) 
//THIS IS MY SOLUTION date(27.10.24)
#include<stdio.h>
#include<windows.h>
#include<string.h>
#include<iostream>
...
{
int i=rsrc.find(".");
cout<<"I AFTER find i:("<<i<<")"<<endl;
cout<<"rsrc.c_str()+i :("<<rsrc.c_str()+i<<")"<<endl;
...
cout<<"@@@@  AFTER PTR ON REQUEST calling ENDS\n";
while(rqs.isForward==1) 
{
//bifargateRequest(rqs);
cout<<"@@@@  rqs.isForward=1 \n";
string rsrc=rqs.forwardedResourceName;
cout<<"@@@@  rsrc=rqs.forwardedResourceName after \n";
cout<<"@##@@##@@## :("<<rsrc<<")"<<endl;
if(this->ResourceCheck(rsrc)) 
{
cout<<"After calling forward resource check DONE string:("<<rsrc<<")"<<endl;
this->ptrOnRequest(rqs,rs);
}
else
{
cout<<"SEND HTML ELSE BLOCk\n";
if(isHTML(rsrc)) 
{
cout<<"SEND HTML GOT CALLED\n";
sendHTML(rsrc,rs);
