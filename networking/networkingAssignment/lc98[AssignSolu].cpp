...
//I COMPLETE THE NECCESSARY PART OF ASSIGNEMENT
#include<stdio.h>
#include<windows.h>
#include<string.h>
#include<iostream>
...
strcat(header,responseBufferLength);
strcat(header,"\n");
}
}
else 
...
result=send(clientSocketDescriptor,header,strlen(header)+1,0);
if(result>0) 
{
printf("Header sended\n");
}
result=send(clientSocketDescriptor,responseBuffer,strlen(responseBuffer)+1,0);
if(result>0)
{
printf("responseBuffer data sended\n");
}
